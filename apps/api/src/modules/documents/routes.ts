...
export async function documentRoutes(server: FastifyInstance) {
  ...
  server.post('/students/:id/documents/upload-url', {
    preHandler: [authMiddleware, requireRole('admin', 'counsellor'), validateParams(schema.idParam), validateBody(schema.uploadUrlSchema)],
    handler: ctrl.requestUploadUrl,
  })
  server.post('/students/:id/documents/complete', {
    preHandler: [authMiddleware, requireRole('admin', 'counsellor'), validateParams(schema.idParam), validateBody(schema.completeUploadSchema)],
