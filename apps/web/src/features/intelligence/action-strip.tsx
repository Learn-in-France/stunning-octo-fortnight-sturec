...
export function ActionStrip({ lead, onChanged }: { lead: ActionStripLead; onChanged?: () => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1" onClick={(e) => e.stopPropagation()}>
      ...
      <Button size="sm" variant="secondary" onClick={openGate}>Gate</Button>
      <Button size="sm" variant="secondary" onClick={() => { setOutcome('not_interested'); setOutcomeOpen(true) }}>Outcome</Button>

      {/* 6Q gate modal */}
      <Modal open={gateOpen} onClose={() => setGateOpen(false)} title={`Qualification gate — ${lead.firstName}`}>
        <div className="space-y-2">
          ...
          <div className="flex justify-end gap-2 pt-3">
            <Button variant="secondary" onClick={() => setGateOpen(false)}>Cancel</Button>
